...
std_residuals = models %>%
  ...
  # raw_residuals = models %>%
  #   mutate_at(.vars=vars(ends_with('model')),
  #             .funs = funs(aug = map(.,augment))) %>%
  #   mutate_at(.vars=vars(ends_with('aug')),
  #             .funs = funs(std_resid = map(.,'.std.resid'),
  #                          raw_resid=map(.,'.resid'))) %>%
  #   select(Gene, data, ends_with('raw_resid')) %>%
  #   unnest() %>%
  #   group_by(Strain, Gene, Condition) %>%
  #   # should only apply to WT
  #   summarize_at(.vars=vars(ends_with('resid')), mean) %>%
  ...
  #   mutate(sample_id = paste(Condition,Strain, sep = '_')) %>%
  #   select(Gene, sample_id, ends_with('resid')) %>%
  #   rename(Strain = Strain_model_aug_raw_resid,
  #          Condition = Condition_model_aug_raw_resid,
  #          Full = Full_model_aug_raw_resid,
  #          Base = Base_model_aug_raw_resid) %>%
  #   gather(residual_type, residual, -sample_id, -Gene)
...
```{r get_weights}
weights = models %>%
  select(Gene, data, Strain_model) %>%
  mutate(weights = Strain_model %>% map(tidy)) %>%
  select(Gene, weights) %>%
  unnest() %>%
  select(Gene, term, estimate) %>%
  spread(key = term,value = estimate)

w_mat = as.matrix(weights %>% select(-Gene))
#d = 1 - cor(t(w_mat))
d = as.matrix(dist(w_mat))
rownames(d) = weights$Gene
colnames(d) = weights$Gene
h = hclust(as.dist(d))
clus = cutree(h,k=30)
clus = tibble(Gene = names(clus), cluster = clus)
clus = left_join(clus, t2g)
```
