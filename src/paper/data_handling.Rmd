...
```{r}
remove_weird_samples = function(resid, cutoff_height = 160, plot=FALSE){
    ...
    if(plot){
        ...
        par(mar = c(0,4,2,0))
        plot(sampleTree,
             main = "Sample clustering of all data to detect outliers",
             sub="",
             xlab="",
             cex.lab = 1.5,
             cex.axis = 1.5,
...
# We squash all residuals that are 2 $\sigma$ down to zero
resid[abs(resid)<2] = 0
resid = remove_weird_samples(resid, plot=TRUE)
...
```{r}
pick_power = function(resid, network_type, powers = c(1:40), plot=FALSE){
    library(WGCNA)
...
edge_power = pick_power(resid, network_type)
