...
Lets sort the genes into clusters by clustering on significantly large residuals:
```{r}
library(WGCNA)
...
write_genes = function(df){
  print(class(df))
  write(df$name, file = paste0('WGCNA_modules/cluster_', 
                               unique(df$module),
                               '.txt'))
}
