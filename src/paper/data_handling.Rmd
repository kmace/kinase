...
```{r}
genes %>% select(Gene_mean, performance) %>% unnest() %>% ggplot(aes(x=Gene_mean, y = r.squared)) + geom_point() + geom_vline(xintercept = 6) # TODO change this all to 5.5
```
...
# In order to remove everything exept for cetrain variables run the following
# rm(list=setdiff(ls(),
#                 c("datExpr",
#                   "sft",
#                   "network_type",
...
```{r}
GOenr = GOenrichmentAnalysis(modules$module, yeastORFs = modules$name, organism = 'yeast')

modules = GOenr$bestPTerms[[4]]$enrichment %>%
            group_by(module) %>%
            nest(.key = 'go_data') %>%
            right_join(modules)
```
...
output_dir = '../../intermediate/clusters/WGCNA_modules'
dir.create(output_dir)

library(readr)

write_genes = function(df){
  write(df$name, 
...
modules %>%
  ...
  #map(~ .x %>% dplyr::select(-module)) %>%
lapply(by_module, write_genes)
  #
  #map(~write_genes(.x))#(.x %>% pull(), file = paste0('WGCNA_modules/cluster_',
                    #                        as.character(.y),
                    #                        '_genes.txt')))
```
...
```{bash}
cd ../../
bash src/scripts/meme/run_all.sh intermediate/clusters/WGCNA_modules intermediate/meme/WGCNA
```
...
tfs = do.call(rbind,
lapply(dir('../../intermediate/meme/WGCNA_modules/tfs/'),
function(x) {
