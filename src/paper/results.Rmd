...
indicating that although these strains are not similar in a general sense, they are similar in Salt, which happens to be a known activator of the hyperosmotic stress response pathway, which PBS2 and HOG1 are members of.


```{r, message = FALSE, fig.width=12, fig.height=10}
tsne_plot(sample_tsne)
```
Notice that some clusters are consistently a single condition, whereas others are consistantly a particluar kinase, and in some cases, the samples are the same condition, and only include kinases that are in the same pathway.
The groups are annotated below.
...
## Expression error  in the linear model modules suggest context dependent regulation.
Recovering known crosstalk and uncovering potential new context dependent regulation.

The linear model has an underlying assumption that the change gene expression chages can be explained by adding up the the contribution of condition, and kinase inhibiotion independently.
Therefore, genes and samples where this prediction fails are strong candidates for an interaction term between Condition and Kianse.

We interpret that interaction as a context dependent event between the codition and kinase.
Therefore identifying known, and new pathways of information propogation from condition to Kinase to gene.
...
<!-- 1.
If a Condition's effect propagates through a inhibition given Kinase, then the linear model will overestimate gene expression for that Condition-Kinase pair   -->
<!--     * HSP12 on Salt + PKA (Get GD?) -->
<!-- 2.
If a Kinase's activity is required to correctly regulate a gene's expression in a condition specific manner, the linear model will badly estimate that condition specific interaction (as there is no term to correct for such a fine tuned response) -->
<!-- 	  * Mating pathway   -->
