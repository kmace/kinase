...
```
Generating a gene expression heatmap across all samples, we can see that there is definite structure. The largest structures seem to be defined by Condition, However smaller structures are hinted at on a kinase level. To explore these further, we use unsupervised methods to cluster the samples
...
lets look at the genes and weights clusterd in a heatmap.
...
## Expression error  in the lienar model modules suggest context dependent regulation. Recovering known crosstalk and uncovering potential new context dependent regulation.
