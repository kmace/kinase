...
We construct a lnear model, that takes in two input variables, `Condition` and `Kinase` and applies a ordinary least squares regression to predict the change in `Expression` from the reference state as an output variable (reference state is `Condition = YPD` and `Kinase = None`).
...
## Residual in the linear model modules suggest context dependent regulation.

So we know that $R_{ij} = I_{ij} + \epsilon_{ij}$

If $\epsilon_{ij}$ is small, then $R_{ij} = I_{ij}$

If $\epsilon_{ij}$ is large, then we can still get around it becuase $\bar{\epsilon_{ij}} \approx 0, when averaged over many genes or:

$\bar{R_{ij}} = \bar{I_{ij}}$

Recovering known crosstalk and uncovering potential new context dependent regulation.

The linear model has an underlying assumption that the change gene expression chages can be explained by adding up the the contribution of condition, and kinase inhibition independently.
