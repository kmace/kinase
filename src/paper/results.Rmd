...
We collect 300 whole genome gene expression measurements, each with one of 10 conditions, and one of 28 kinases inhibited. Clustering of these samples through t-Distributed Stochastic Neighbor Embedding (t-SNE) reveals
...
Firstly, we look at samples that have no analog-sensitive kinase:
...
```
Notice that some clusters are consistantly a single condtion, whereas others are consistantly a particluar kinase, and in some cases, the samples are the same condtion, and only include kinases that are in the same pathway.
