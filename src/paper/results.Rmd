...
TODO: try make this heatmap look better with morpheus.

Generating a gene expression heat-map across all samples, we can see that there is definite structure. The largest structures seem to be defined by Condition, However smaller structures are hinted at on a kinase level. To explore these further, we use unsupervised methods to cluster the samples

### Clustering samples reveals distinct condition, kinase, and pathway specific groups

We collect 300 whole genome gene expression measurements, each with one of 10 conditions, and one of 28 kinases inhibited. Clustering of these samples through t-Distributed Stochastic Neighbor Embedding (t-SNE) reduces these samples from ~6000 dimensions (genes) into 2. Although this representation can not be used to infer proporites of the global structure of the sample space, it can be used to identify clusters of similar samples (gene expression profiles)

```{r ref.label='tsne_cluster', eval=FALSE}
```


When plotting each sample in t-SNE space, we see that the samples cluster into distinct kinase, and condition, and context dependent clusters:

Firstly, we will look at the bioligical replicates in the collected data, 3-4 wild-type strains subjected to one of the ten possilbe conditions.
...
Now we look at all the data. We see that samples are generally clustering together based on their condition, or strain. This indicates that there are strong, reproducible signals generated at the condition and kinase levels. Interestingly, we also notice that PBS2 and HOG1, do not cluster with other salt samples, but rather cluster together. We call this a pathway specific cluster. indicating that although these strains are not similar in a general sense, they are similar in Salt, which happens to be a known activator of the hyperosmotic stress response pathway, which PBS2 and HOG1 are members of.


```{r, message = FALSE, fig.width=12, fig.height=10}
tsne_plot(sample_tsne)
```
Notice that some clusters are consistantly a single condtion, whereas others are consistantly a particluar kinase, and in some cases, the samples are the same condtion, and only include kinases that are in the same pathway. The groups are annotated below.
...
3. specific instances of condition + kinase-inhibition pairs that do not conform to the addition of their general effects

To achieve this we create a linear model for each gene, using the `Condition` and `Kinase` as predictor variables to predict `Expression` as an output variable (The intercept is also included as a predictor variable).
As linear models require a reference level for each predictor factor, The baseline `Kinase` is the wild-type strain and the base condition is YPD.
...
The Condition weight indicates the best approximation to the change that a non-YPD condition will have on a gene. (over all kinases)
...
Model performance is also assessed to see if a simplistic additive linear (with no interaction term) is capable of modeling the majority of the signal. turns out it does pretty well, see supplement for an in-depth analysis.

## Condition weights identify generally accepted gene expression responses

How am i going to show this? is it important?

lets look at the genes and weights clustered in a heat-map.

It is widely accepted that there are condition specific gene expression responses. I don't think we have anything new to say here, other than our data also shows this and therefore the gene expression profiles are to be believed.
...
condition_terms %>%
  ...
  heatmap3::heatmap3(condition_terms_matrix)
...
Basically. I want to start to introduce the logic behind kinase inhibition. If the kinase is off in YPD, we do not expect there to be a significant gene expression response when we inhibit it. I think one thing we can make a strong claim about is how does a condition modulate a kinase. we have all the info. first we start with the basal level here. then later we use the residual to define context dependent stuff.
...
kinase_terms_matrix = kinase_terms_matrix[apply(kinase_terms_matrix,1,max)>0,]
  heatmap3::heatmap3(kinase_terms_matrix)

```


## Expression error  in the linear model modules suggest context dependent regulation. Recovering known crosstalk and uncovering potential new context dependent regulation.

The linear model has an underlying assumption that the change gene expression chages can be explained by adding up the the contribution of condition, and kinase inhibiotion independently. Therefore, genes and samples where this prediction fails are strong candidates for an interaction term between Condition and Kianse.

We interpret that interaction as a context dependent event between the codition and kinase. Therefore identifying known, and new pathways of information propogation from condition to Kinase to gene.


In order to account for noise in measurements, we further cluster genes wit similar context depedent regulation into modules. Hoping to show that these modules have Enrichment for GO Terms, and TF motifs in their promoters.
