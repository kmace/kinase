...
Kinases are largely responsible for mediating environmental information to the nucleus where an adaptive transcriptional response can be made. The propagation of this information occurs through a pathways of cascading phosphorylation events. Here we use strains with analog sensitive mutants of certain stress associated kinases to block kinase signaling while under a variety of stress. The resulting change in gene expression is then measured using RNA-seq and analyzed.
...
### Calculation of gene statistics, and filtering:
...
Next we create a linear model for each gene, using the `Condition` and `Kinase` as predictor variables to predict `Expression` as an output variable. The linear model has the form `Expression ~ Kinase + Condition + Intercept`.

We also simultaniously collect model weights, model performance, and residuals.
