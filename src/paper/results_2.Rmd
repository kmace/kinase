...
For every gene, we build a linear model to predict gene expression. The inputs to this model are the sample metadata (Condition and Kinase inhibiiton). Although sometimes unrealistic, these models assume that changes in gene expression can be computed by adding up the individual effects provided by the condition, and from the kinase.  For example:
...
Where $\Delta E$ is the change in log expression, and $C$ and $K$ are the contributions from the condition and kinase respectivly. 
