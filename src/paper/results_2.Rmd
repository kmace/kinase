...
```{r}
expression_plot('ATP8', measurements, t2g)
residual_plot('ATP8', std_residuals, t2g)
```

Looks like our filtering is bad:

```{r}
out = genes %>% select(starts_with('Gene')) %>% summarize_all(mean) %>% left_join(model_quality %>% filter(model_type == 'Full') %>% select(Gene, r.squared))

ggplot(out,aes(y = r.squared, x = Gene_mean)) + geom_point()
```
